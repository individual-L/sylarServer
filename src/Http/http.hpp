...
//获取map中的val值，并将其转换成相应类型,失败返回false
template<class TypeMap, class T>
bool checkGetAs(const TypeMap& m, const std::string& key, T& val, const T& def = T()) {
    auto it = m.find(key);
    if(it == m.end()) {
        val = def;
        return false;
...
class HttpRequest{
  ...
  typedef std::map<std::string,std::string,Uncasecmp> TypeMap;
    ...
    const TypeMap& getHeaders() const { return m_headers;}
    ...
    const TypeMap& getCookies() const { return m_cookies;}

    const TypeMap& getParams() const { return m_params;}
    ...
    void setHeaders(const TypeMap& v) { m_headers = v;}
    ...
    void setParams(const TypeMap& v) { m_params = v;}

    void setCookies(const TypeMap& v) { m_cookies = v;}
